...
namespace xAudio
{
	class AudioStream : public cocos2d::Ref
	{
	public:
	...
	template<>
	class AudioStreamAdapter<uint8_t*> : public AudioStream
	{
		uint64_t _pos = 0;
		uint8_t* _buffer;
		uint64_t _size;
		std::mutex _mut;
	public:
		explicit AudioStreamAdapter(uint8_t* buffer, uint64_t size) : _buffer(buffer),_size(size)
		{
			autorelease();
		}
