...
#define LUA_TRY_INVOKE(ARGC_, Fn_)\
	if (argc == (ARGC_) && !ok) {\
		...
		lua_insert(lua_S, 1);\
		ok = audiolua::invoke_native(lua_S, arg_cur, (Fn_));\
		if (ok) {\
			lua_settop(lua_S, 1);\
			return 1;\
		} else {\
...
#define LUA_CLS_END()\
	...
	audiolua::registerLuaType<current_type>(typeName, castName);
#endif // !LUA_CLS_END
...
#define LUA_PUSH_NATIVE(V_)\
	audiolua::native_to_luaval(lua_S, V_);
#endif // !LUA_PUSH_NATIVE

#ifndef LUA_NATIVE_SETTER
#define LUA_NATIVE_SETTER(V_, IDX_)\
	if (!audiolua::luaval_to_native(lua_S, (IDX_), &(V_), LUA_FNAME))\
